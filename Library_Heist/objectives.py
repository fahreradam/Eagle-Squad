...
class Bookshelves:
    ...
    def read(self, player):
        if self.book >= 1 and not self.read_book:
            if self.tables[0].collidepoint(player[0] + 25, player[1] + 25) or \
                    self.tables[1].collidepoint(player[0] + 25, player[1] + 25) or \
                    self.tables[2].collidepoint(player[0] + 25, player[1] + 25) or \
                    self.tables[3].collidepoint(player[0] + 25, player[1] + 25) or \
                    self.tables[4].collidepoint(player[0] + 25, player[1] + 25) or \
                    self.tables[5].collidepoint(player[0] + 25, player[1] + 25):
                if pygame.key.get_pressed()[pygame.K_e]:
...
class Bathroom:
    ...
    def number2(self, player):
        if self.toilet[self.selection].collidepoint(player[0] + 25, player[1] + 25) and not self.flush:
